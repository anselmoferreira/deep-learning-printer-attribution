...
for prt1 = 1 : (nprts - 1);
	...
	%the default values used here were tolkkt=0.1 and kktviolationlevel=0 for all the other techniques	
        ...
        %passo1: aumentar núemro de iterações.
        %passo 2: diminuir número de iterações, mas colocar parâmetros diferentes, como no Luiz.
        optSVM = statset('MaxIter', 10000000);
	%'Display','iter'

	%Please use these parameters for techniques which not converge (GLCM_MD_MS_C for example)
        %PARA QUEM NÃO CONVERGE
       SVMtrainModel = svmtrain(TrainData, TrainBin, 'kernel_function', 'linear', ...
            'options', optSVM, 'tolkkt', 0.5, 'kktviolationlevel', 0.1, 'kernelcachelimit', 30000);
 


       %SVMtrainModel = svmtrain(TrainData, TrainBin,'kernel_function', 'linear', ...
        %    'options', optSVM, 'tolkkt', 0.01, 'kernelcachelimit', 30000);

	%PARA QUEM CONVERGE OP1
         %SVMtrainModel = svmtrain(TrainData, TrainBin, 'kernel_function', 'linear', ...
